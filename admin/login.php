...
//    $fbanned = fopen("banned_ip.txt", "r") or die("Unable to open file!");
//    while(!feof($fbanned)) {
//      $ipbanned = fgets($fbanned);
...
if (isset($_POST["btnLogin"]) && token()) {
	if (filter_var($_POST["txtEmail"], FILTER_VALIDATE_EMAIL) === false) {
		$error[] = "Đây không phải là Email hoặc Email không hợp lệ.";
	}
