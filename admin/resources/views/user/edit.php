	...
	if ($_SESSION["login"]["kt3V_id"] != 1 && ($id == 1 || ($data["level"] == 3 && $edit_myself == false))) {
		...
		if ((isset($_POST["btnSave"]) || isset($_POST["btnSaveClose"])) && token()) {
			if (checkEmpty($_POST["txtPass"])) {
				...
				$pass = password_hash($data["email"]."@-*$".$_POST["txtPass"],PASSWORD_DEFAULT, ['cost' => 12]);
			}
			...
			<div class="panel-body" style="display: block;">
				...
				<div class="form-group">
					...
					<div class="col-lg-9" style="padding-left:0px">
						...
						<input type="text" class="form-control" placeholder="Enter your password" name="txtPass" >
			...
			<div class="panel-body" style="display: block;">
				...
				<div class="form-group">
					...
					<center>
						...
						<input type="hidden" name="txtImage" id="main-image-input" <?php issetInputImage('txtImage',$data["avatar"],'value') ?> />
					</center><br />
				</div><br />
				<div class="checkbox checkbox-switch">
					<input type="checkbox" name="chkStatus" data-on-color="success" data-off-color="danger" data-on-text="Status Online" data-off-text="Status Offline" class="switch" data-table="u8tr_user" data-col="status" data-id="<?php echo $data["id"] ?>" <?php echo ($data["status"] == "On") ? "checked" : "" ?> />
				</div>
			</div>
		</div>
	</div>
