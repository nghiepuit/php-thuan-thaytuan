...
function recursionCategory($data, $parent = 0, $str = '')
{
    foreach ($data as $key => $value) {
        $id = $value["id"];
        $category_id = $value["category_id"];
        $category_name = $value["name_vi"];
        $course_id = $value["course_id"];
        $parent_id = $value["parent_id"];
        if ($parent_id == $parent) {
            echo '<a href="index.php?con=category&act=edit&cid=' . $category_id . '">' . $str . $category_name . ' </a></br>';
            recursionCategory($data, $category_id, $str . $category_name . ' > ');
        }
    }
}
...
function recursionSelect($data, $selected = 0, $parent = 0, $str = '')
{
    foreach ($data as $key => $value) {
        ...
        $name_en = $value["name_en"] != "" ? " (" . $value["name_en"] . ")" : "";
        $parent_id = $value["parent_id"];
        if ($parent_id == $parent) {
            if ($id == $selected) {
                echo '<option value="' . $id . '" selected>' . $str . $name_vi . $name_en . '</option>';
            } else {
                echo '<option value="' . $id . '">' . $str . $name_vi . $name_en . '</option>';
            }
            unset($data[$key]);
    ...
    foreach ($data as $key => $value) {
        ...
        if ($parent_id == $parent) {
			...
			<tr>
				...
				<td>' . $str . ' <a href="index.php?con=category&act=edit&cid=' . $id . '">' . $name_vi . $name_en . '</a></td>
				<td><input type="checkbox" name="chkStatus" data-on-color="success" data-off-color="danger" data-on-text="Online" data-off-text="Offline" class="switch" data-table="u8tr_category" data-col="status" data-id="' . $id . '" ' . $status . ' /></td>
				<td class="text-center">
					<ul class="icons-list">
						<li class="text-primary-600"><a href="index.php?con=category&act=edit&cid=' . $id . '" data-popup="tooltip" title="Edit"><i class="icon-pencil7"></i></a></li>
						<li class="text-danger-600"><a href="index.php?con=category&act=delete&cid=' . $id . '" data-popup="tooltip" title="Remove" class="sweet_warning"><i class="icon-trash"></i></a></li>
					</ul>
				</td>
			</tr>';
            unset($data[$key]);
    ...
    if ($child) {
        ...
        foreach ($child as $key => $value) {
            ...
            if (!empty($checked) && in_array($id, $checked)) {
                $input = '<input type="checkbox" name="chkCategory[]" value="' . $id . '" checked /> ' . $name_vi . $name_en;
            } else {
                $input = '<input type="checkbox" name="chkCategory[]" value="' . $id . '" /> ' . $name_vi . $name_en;
            }
